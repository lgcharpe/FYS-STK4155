...
def Ridge_FF(X, Y, F, lam=0.01, k=1, graph=True):
    ...
    if(graph):
        ...
        plt.title("Ridge Regression polynomial order %d with lambda %.04f" % (k, lam))
        plt.xlabel('X')
        plt.ylabel('Y')
...
def boot_Ridge_FF(X, Y, F, lam=0.01, it=1000, x_k=1):
    ...
    for i in range(it):
        ...
        MSE, R2, var = Ridge_FF(Xt, Yt, Ft, lam, x_k, False)
        tMSE = tMSE + MSE
        tR2 = tR2 + R2
        tvar = tvar + var
        aMSE[i] = MSE
...
def k_fold_CV_Ridge_FF(X,Y,F,lam=0.01,k=5,x_k=1):
...
place = 0
for i in lambdas:
    MSE_5, R2_5, var_5 = Ridge_FF(X, Y, F, lam=i, k=5, graph=False)
    MSEs[place] = MSE_5
    R2s[place] = R2_5
    place += 1
...
for x_k in range(1,6):
    MSE, R2, var = Ridge_FF(X, Y, F, k=x_k, lam=0.0012, graph=False)
    nMSE[x_k-1] = MSE
    nR2[x_k-1] = R2
    if(nR2[bk] < nR2[x_k-1]):
        bk = x_k
...
cvMSE_test, cvR2_test, cvMSE_train, cvR2_train = k_fold_CV_Ridge_FF(X,Y,F,lam=0.0012,k=k,x_k=bk)
